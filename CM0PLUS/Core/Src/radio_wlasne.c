...
void HAL_SUBGHZ_RxCpltCallback(SUBGHZ_HandleTypeDef *hsubghz)
{
	...
	chErr |= PobierzStatusBufora(&chStatus, &chIloscOdebranychDanych, &chWskaznikDanych);
...
//////////////////////////////////////////////////////////////////////////////////
// pobiera status bufora odbiorczego
// Parametry:
//	*chStatus - wskaźnik na status radia
//	chBudOdb - offset adresu bufora odbiornika względem adresu bazowego sub-GHz RAM
// Zwraca: kod błędu
...
uint8_t WlaczObiorGFSK(uint32_t nTimeout)
{
	...
	int8_t chRssiPakietu;
	int8_t chRssiSync;
	uint16_t sTimeout, sStatusPrzerwania = 0;
	...
	uint8_t SyncWord[8] = {0xC1, 0x94, 0xC1, 0xC1, 0xC1, 0xC1, 0xC1, 0xC1};
	...
	chErr = UstawParametryPakietowGFSK(16, 4, 20);		//sDlugPreamb, chSyncWordlLength, chPayloadLenght

	//4. Define synchronization word in the associated packet type SUBGHZ_xSYNCR(n) with Write_Register().
	chErr = HAL_SUBGHZ_WriteRegisters(&hsubghz, SUBGHZ_GSYNCR0, SyncWord, 8);	//SUBGHZ_GSYNCR0
...
uint8_t WyslijRamkeGFSK(void)
{
	...
	chErr |= UstawParametryPakietowGFSK(64, 4, 22);	//sDlugPreamb, chSyncWordlLength, chPayloadLenght
